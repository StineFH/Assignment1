...
##### Cross validation with VerbalIQ #####
performances <- c() # We create an empty vector for the performances
#### loop for running cross validation ####
for (k in 1:max(df_train$fold)){ 
  fold_k <- as.integer(as.character(k)) # Convert k to integer 
...
#Creating ASD dataframe
asd <- subset(df_train, Diagnosis == "ASD")

#Creat a model with ASD data 
m4 <- lmer(CHI_MLU ~ Visit*verbalIQ1 + (1 + Visit|Child.ID), data = asd, REML =F, lmerControl(optCtrl=list(xtol_abs=1e-8, ftol_abs=1e-8)))

#Comparing Bernie to children with ASD 
pred_asd <- predict(m4, bernie, allow.new.levels = T)
pred_asd
bernie$CHI_MLU[6]-pred_asd[[6]]

#Creating model with TD children 
m3 <- lmer(CHI_MLU ~ Visit*verbalIQ1 + (1 + Visit|Child.ID), data = td, REML =F, lmerControl(optCtrl=list(xtol_abs=1e-8, ftol_abs=1e-8)))

#Comparing Bernie to other TD children 
pred_td <- predict(m3, bernie, allow.new.levels = T)
pred_td
bernie$CHI_MLU[6]-pred_td[[6]]
